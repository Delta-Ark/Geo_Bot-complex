...
class GeoSearchClass(object):
    ...
    def count(self, value):
        ...
        if isinstance(value,(float,int)):        
            if not (value>0 and value<101 and value==int(value)) :
                raise ValueError("count is '" + str(value) + "' but count must be an integer and 0 < count < 101")
        self._count = value
    ...
    def result_type(self, rt):
        if not (rt=="mixed" or rt=="popular" or rt=="recent"):
            raise ValueError("result_type must be 'mixed', 'recent', or 'popular' NOT '"+str(rt)+"'")
    ...
    def longitude(self,value):
        ...
        if not (value>-180.0 and value<180.0):
            raise ValueError("longitude must be in bounds: 180.0>longitude>-180.0")
    ...
    def geo_string(self):
        ...
        self._geo_string=str(self._latitude) + "," + str(self._longitude) + "," + str(self._radius)+"mi"
