...
def train_pcfg():
    ...
    trees = []
    for fileid in treebank.fileids()[0:20]: # up to 199 less for shorter grammar for quicker training
        for tree in treebank.parsed_sents(fileid):
            # perform optional tree transformations, e.g.:
            #  Remove branches A->B->C into A->B+C so we can avoid infinite productions
...
def traverse_tree_grab_phrase(tree, label):
    ...
    for subtree in tree:
        ...
        if issubclass(subtree.__class__, nltk.tree.Tree):
            logging.debug( 'this subtree has label {}'.format(subtree.label()))
            logging.debug( 'subtree {} == label {} : {}'.format(subtree.label(), label, subtree.label() == label))
...
def create_info_phrase_add_to_list(phrases, status, dict_list):
    keys = ['phrase', 'phrase_type', 'tweet', 'coordinates', 'time', 'screen_name']
