...
def growing_plot(geosearchclass, number_of_words):
    ...
    #samples.reverse()  # want most frequent on top -> doesn't work for extension
    ...
    for i in xrange(100):
        ...
        if new_search_results:
            ...
            newsamples = [item for item, _ in fdist.most_common(number_of_words)]
            s1 = set(newsamples)
            s2 = set(samples)
...
def static_updating_plot(geosearchclass, number_of_words):
    search_results = geosearchclass.search()
    filtered_words = vis_helper.process(search_results)
    fdist = vis_helper.get_freq_dist(filtered_words)
    #set up plot
